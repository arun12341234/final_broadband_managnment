    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-500 to-orange-700 flex items-center justify-center px-4">
      <Card className="w-full max-w-md p-8">
        ...
        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <p className="text-xs text-blue-800">Password: Check your backend/.env file</p>
        </div>
      </Card>
    </div>
  );
      ...
      {engineers.length === 0 ? (
        <Card className="p-12">
          <EmptyState
            ...
            description="Add your first field engineer to get started"
          ...
          {engineers.map((engineer) => (
            <Card key={engineer.id} className="p-6 hover:shadow-lg transition-shadow">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <Badge variant={engineer.status === 'Active' ? 'success' : 'warning'}>
                      {engineer.status}
                    </Badge>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                ...
                {engineer.joining_date && (
                  <div className="flex items-center gap-2 text-sm text-gray-700">
                    <Calendar className="w-4 h-4 text-gray-400" />
                    <span>Joined: {engineer.joining_date}</span>
                  </div>
                )}
                {engineer.emergency_contact && (
                  <div className="flex items-center gap-2 text-sm text-gray-700">
                    <AlertTriangle className="w-4 h-4 text-gray-400" />
                    <span>Emergency: {engineer.emergency_contact}</span>
                  </div>
                )}
              </div>
              ...
              {engineer.address && (
                <div className="mt-4 pt-4 border-t border-gray-200">
                  <div className="flex items-start gap-2 text-xs text-gray-600">
                    ...
                    <span className="line-clamp-2">{engineer.address}</span>
                  </div>
                </div>
              )}
            </Card>
...
const AddEngineerModal = ({ onClose, onSuccess, showToast }) => {
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Add New Engineer" size="max-w-2xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Name */}
          <div>
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
              placeholder="Fiber Installation"
            />
          </div>
...
const EditEngineerModal = ({ engineer, onClose, onSuccess, showToast }) => {
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Edit Engineer" size="max-w-2xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t">
          ...
          <Button type="submit" disabled={loading}>
            {loading ? <span className="flex items-center gap-2"><Loader className="w-4 h-4 animate-spin" />Updating...</span> : 'Update Engineer'}
          </Button>
        </div>
      </form>
    </Modal>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Plan Change</th>
                ...
                {filteredHistory.map((record) => {
                  ...
                  return (
                    <tr key={record.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Badge variant="info">{record.change_type}</Badge>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        {record.previous_payment_status !== record.new_payment_status && (
                          <div className="flex items-center gap-2">
                            <span className="text-gray-500">{record.previous_payment_status}</span>
                            <span>→</span>
                            <span className="font-medium">{record.new_payment_status}</span>
                          </div>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        {record.previous_plan_name !== record.new_plan_name && (
                          <div className="flex items-center gap-2">
                            <span className="text-gray-500">{record.previous_plan_name}</span>
                            <span>→</span>
                            <span className="font-medium">{record.new_plan_name}</span>
                          </div>
                        )}
                      </td>
...
const NotificationsTab = ({ users, plans, showToast }) => {
  ...
  const handleSendNotifications = async () => {
    if (!window.confirm('Send all scheduled notifications now?')) return;
  ...
  const handleCheckExpiredPlans = async () => {
    if (!window.confirm('Check and expire all expired plans now?')) return;

    setLoading(true);
    try {
      const response = await api.post('/api/users/check-expired-plans');
      showToast(`${response.data.expired_count} plans expired`, 'success');
    } catch (error) {
  ...
  const pendingPayments = users.filter(u => u.payment_status === 'Pending');
  const expiringToday = users.filter(u => {
    const today = new Date().toISOString().split('T')[0];
    return u.plan_expiry_date === today;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">Manage system notifications and reminders</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Send All Notifications</h3>
              <p className="text-sm text-gray-600">Trigger all scheduled notifications manually</p>
            </div>
          </div>
          <Button onClick={handleSendNotifications} disabled={loading} className="w-full">
        ...
        <Card className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Manually trigger plan expiry check</p>
            </div>
          </div>
          <Button onClick={handleCheckExpiredPlans} disabled={loading} className="w-full" variant="danger">
            {loading ? 'Checking...' : 'Check Plans'}
          </Button>
        </Card>
      </div>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-orange-600 mt-1">{expiringToday.length}</p>
            </div>
      ...
      <Card className="p-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4 p-4 bg-blue-50 rounded-lg">
            ...
            <div className="flex-1">
              <h4 className="font-medium text-gray-900">12:05 AM - Plan Expiry Check</h4>
              <p className="text-sm text-gray-600">Automatically expire plans and reset payment status</p>
            </div>
          ...
          <div className="flex items-center gap-4 p-4 bg-green-50 rounded-lg">
            ...
            <div className="flex-1">
              <h4 className="font-medium text-gray-900">2:00 AM - Database Backup</h4>
              <p className="text-sm text-gray-600">Create daily backup of all data</p>
            </div>
          ...
          <div className="flex items-center gap-4 p-4 bg-orange-50 rounded-lg">
            ...
            <div className="flex-1">
              <h4 className="font-medium text-gray-900">10:00 AM - Send Notifications</h4>
              <p className="text-sm text-gray-600">Email & WhatsApp reminders for payments and expiry</p>
            </div>
...
const SettingsTab = ({ showToast }) => {
  ...
  const fetchSystemConfig = async () => {
    try {
      const [emailRes, whatsappRes, dbRes] = await Promise.all([
        api.get('/api/system/test-email'),
        api.get('/api/system/test-whatsapp'),
  ...
  const handleTestEmail = async () => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        showToast('Email configuration is working!', 'success');
      } else {
  ...
  const handleTestWhatsApp = async () => {
    ...
    try {
      ...
      if (response.data.status === 'configured') {
        showToast('WhatsApp configuration is working!', 'success');
      } else {
        showToast(response.data.message, 'warning');
      }
      setWhatsappConfig(response.data);
    } catch (error) {
      showToast('Failed to test WhatsApp', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">Test and manage system settings</p>
      </div>
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">SMTP settings for notifications</p>
            </div>
          </div>
        ...
        {emailConfig && (
          <div className={`p-4 rounded-lg ${
            emailConfig.status === 'success' ? 'bg-green-50 border border-green-200' :
            emailConfig.status === 'disabled' ? 'bg-gray-50 border border-gray-200' :
            'bg-yellow-50 border border-yellow-200'
          }`}>
            <p className="text-sm font-medium text-gray-900">{emailConfig.message}</p>
          </div>
        )}
      </Card>
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">WhatsApp Business API / Twilio</p>
            </div>
          </div>
          <Button onClick={handleTestWhatsApp} disabled={loading}>
            Test WhatsApp
          </Button>
        </div>
        ...
        {whatsappConfig && (
          <div className={`p-4 rounded-lg ${
            whatsappConfig.status === 'configured' ? 'bg-green-50 border border-green-200' :
            whatsappConfig.status === 'disabled' ? 'bg-gray-50 border border-gray-200' :
            'bg-yellow-50 border border-yellow-200'
          }`}>
            <p className="text-sm font-medium text-gray-900">{whatsappConfig.message}</p>
            {whatsappConfig.provider && (
              <p className="text-xs text-gray-600 mt-1">Provider: {whatsappConfig.provider}</p>
            )}
          </div>
        )}
      </Card>
      ...
      <Card className="p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">System database details</p>
          </div>
        </div>
        ...
        {dbInfo && (
          <div className="grid grid-cols-2 gap-4">
            <div className="p-3 bg-gray-50 rounded-lg">
              <p className="text-xs text-gray-600">Database Type</p>
              <p className="text-sm font-medium text-gray-900">{dbInfo.type}</p>
            </div>
            <div className="p-3 bg-gray-50 rounded-lg">
              <p className="text-xs text-gray-600">Total Users</p>
              <p className="text-sm font-medium text-gray-900">{dbInfo.total_users}</p>
            </div>
            <div className="p-3 bg-gray-50 rounded-lg">
              <p className="text-xs text-gray-600">Total Plans</p>
              <p className="text-sm font-medium text-gray-900">{dbInfo.total_plans}</p>
            </div>
            <div className="p-3 bg-gray-50 rounded-lg">
              <p className="text-xs text-gray-600">Total Engineers</p>
              <p className="text-sm font-medium text-gray-900">{dbInfo.total_engineers}</p>
            </div>
            <div className="p-3 bg-gray-50 rounded-lg">
              <p className="text-xs text-gray-600">Database Size</p>
              <p className="text-sm font-medium text-gray-900">{dbInfo.size}</p>
            </div>
            <div className="p-3 bg-gray-50 rounded-lg">
              <p className="text-xs text-gray-600">Last Backup</p>
              <p className="text-sm font-medium text-gray-900">{dbInfo.last_backup}</p>
            </div>
          </div>
        )}
      </Card>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Download users data as Excel</p>
            </div>
          </div>
      ...
      <Card className="p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">Production security recommendations</p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3 p-3 bg-green-50 border border-green-200 rounded-lg">
            <CheckCircle className="w-5 h-5 text-green-600" />
            <span className="text-sm text-gray-900">✓ Rate limiting active</span>
          </div>
