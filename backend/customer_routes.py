...
async def get_customer_bills(
    ...
    # Add current month bill if payment is pending and no admin invoice exists for this period
    if current_user.payment_status == "Pending":
        # Compute current billing period like admin invoice (Month Year - Month Year)
        today = datetime.now()
        ...
        current_period = f"{billing_start.strftime('%B %Y')} - {billing_end.strftime('%B %Y')}"

        has_invoice_for_period = any(inv.billing_period == current_period for inv in invoices)

        if not has_invoice_for_period:
            bills.append({
                "id": 0,  # Current bill
...
async def get_customer_invoices(
    ...
    """Get all invoices for the current customer (for precise syncing in the portal)"""
    from models import Invoice
    ...
    for inv in invoices:
        result.append({
            ...
            "pdf_available": bool(inv.pdf_filepath),
